...
parser.add_argument('-t', '--task', default="cliff", type=str, help='任务名称, sumo/highway/intersection')
...
def counterfactual_exp_expand(replay_buffer, sta, batch_size, action_space_size, distance_ratio):
    '''
    [离散动作空间的CEA，仅适用于RDQN，连续动作空间请参考 utils/CEA.py]
    replay_buffer: 经验池
    sta: cvae
    batch_size: 抽多少经验
...
if __name__ == '__main__':
    ...
    for seed in trange(args.begin_seed, args.end_seed + 1):
        seed_torch(seed)
